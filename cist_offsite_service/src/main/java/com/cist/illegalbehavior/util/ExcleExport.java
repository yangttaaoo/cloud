...
public class ExcleExport {
     ...
     * @param sheetName
     */
    public static void reportMergeXls(List<HashMap<String,Object>> list,String sheetName,String[] headers,List<Sys_frm_code> list1 ,OutputStream out,String flag) throws Exception{
...
//            font.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);//粗体显示
            ...
            for (int j = headers.length-1; j < list1.size()+headers.length-1; j++) {
                cell = row.createCell((short)j);
                cell.setCellStyle(cs);
                	cell.setCellValue(list1.get(j-(headers.length-1)).getFc_name());
                    sheet.autoSizeColumn(j,true);//自动设宽
...
//            sheet.autoSizeColumn(0, true);// 根据字段长度自动调整列的宽度
            ...
            if("0".equals(flag))
            {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    row.createCell(0).setCellValue((String)map.get("glbm"));
                   
                    for (int j = 0; j < list1.size(); j++) {
                    	row.createCell(j+(headers.length-1)).setCellValue(map.get(list1.get(j).getFc_code())==null?0:Integer.valueOf(map.get(list1.get(j).getFc_code()).toString()).intValue());
        			}
            ...
            else
            {
                for (int i = 0; i < list.size(); i++) {
                    row = sheet.createRow(i + 2);
                    Map map = list.get(i);
                    row.createCell(0).setCellValue((String)map.get("wfddmc"));
                    row.createCell(1).setCellValue(map.get("cjzs")==null?0:Integer.valueOf(map.get("cjzs").toString()).intValue());
                    row.createCell(2).setCellValue(map.get("yxs")==null?0:Integer.valueOf(map.get("yxs").toString()).intValue());
                    row.createCell(3).setCellValue(map.get("wxs")==null?0:Integer.valueOf(map.get("wxs").toString()).intValue());
                    row.createCell(4).setCellValue((String)map.get("yxl"));
                    for (int j = 0; j < list1.size(); j++) {
