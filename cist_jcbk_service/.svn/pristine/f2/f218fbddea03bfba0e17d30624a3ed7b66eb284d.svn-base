...
public class TblCarRecoInfo {
	...
	public String getXsdd() {
		String wfddm = StringUtils.addZeroForFiled(this.ssdl==null||"".equals(this.ssdl)?"0":this.ssdl,5)
				+StringUtils.addZeroForFiled(this.lddm==null||"".equals(this.lddm)?"0":this.lddm,4)
		+StringUtils.addZeroForFiled((this.ldms==null || "".equals(this.ldms))?"0":this.ldms,3);
		xsdd = SysTempStoreUtils.getFcName(SysBaseDataCode.SYS_WFDD, wfddm);
		xsdd=xsdd+(cdfx==null?"":"，"+cdfx)+(cdbh==null?"":"，第"+cdbh+"车道");
		return xsdd;
	}
	...
	public String getRoim_name() {
		if(ssdl!=null) {
			SysTempStoreUtils.getFcName(SysBaseDataCode.SYS_ROAD, ssdl);
		}else {
			roim_name = "未知";
		}
		return roim_name;
