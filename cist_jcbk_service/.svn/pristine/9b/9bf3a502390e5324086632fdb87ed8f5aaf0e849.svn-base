package com.cist.earlyplayback.controller;
...
@RequestMapping("/earlyplayback")
public class EarlyPlaybackController {
	@Autowired
	private WarningMonitorService iservice;
	@Autowired
	private EarlyPlaybackService service;
	@Autowired
	 ...
	 * @param map
	 */
	@RequestMapping("playbackinfoList")
    public PageInfo<MonitorInfo> playbackinfopageList(@RequestBody HashMap<String,Object> modelMap ){
    	...
    	PageInfo<MonitorInfo> list= (PageInfo<MonitorInfo>)service.playbackinfopageList(modelMap,pinfo);
