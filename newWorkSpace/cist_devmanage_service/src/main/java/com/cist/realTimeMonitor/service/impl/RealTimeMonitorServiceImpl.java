...
//		this.initDeviceTree(topDept, DeviceList);
		DeptDeviceTree topDept1 = mapper.queryTopLevelDept();
		List<DeptDeviceTree> topDept = new ArrayList<DeptDeviceTree>();
		topDept.add(topDept1);
		this.getchildrenNode(topDept, DeviceList, null, 0);
	
		for(DeptDeviceTree model:topDept) {
			this.setTopDeptTitle(model, DeviceList);
		}
		
		resultMap.put("deptDevice", topDept.get(0));
		...
		for (DeptDeviceTree dept : deptList) {
			...
			if (null != dept.getPk()) {
				...
				if (childrenDeptList.size() != 0) {
					if (parentDept != null) {
					if (deviceTypeSize > 0) {//该子部门有设备就直接添加为父部门children
					...
					if (childrenDeviceNum > 0) {
						if (parentDept != null) {
							...
							List<DeptDeviceTree> children = parentDept.getChildren();
							boolean contains = children.contains(dept);
							if(!contains) 
								parentDept.getChildren().add(dept);
			...
			if (null != areas.getPk()) {
				...
				if (childrenDeptList.size() != 0) {
					if (parentAdministrativeAreas != null) {
						if (deviceTypeSize > 0) {//该子巡区有设备就直接添加为父区划children
					...
					if (childrenDeviceNum > 0) {
						if (parentAdministrativeAreas != null) {
							...
							//判断是否已经添加为children,若没有就添加
							List<AdministrativeAreasTree> children = parentAdministrativeAreas.getChildren();
							boolean contains = children.contains(areas);
							if(!contains) 
								parentAdministrativeAreas.getChildren().add(areas);
						}
					}
				} else {
					// 该行政区划没有子行政区划时 sizeNum用于统计父行政区划下所有子行政区划设备总数
					sizeNum += deviceTypeSize;
					if (deviceTypeSize > 0) {
						if (parentAdministrativeAreas.getChildren() == null) {
							parentAdministrativeAreas.setChildren(new ArrayList<AdministrativeAreasTree>());
