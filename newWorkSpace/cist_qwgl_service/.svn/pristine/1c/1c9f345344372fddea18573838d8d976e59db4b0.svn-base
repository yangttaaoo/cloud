...
import com.cist.depttestin.model.TargetInfo;
  ...
  public PageInfo<Testin> selectData(@RequestBody HashMap<String, Object> map){
		try {
			...
			for(int i=0;i<info.getList().size();i++){
				if(info.getList().get(i).getFk_topic_id()!=null ){
					TargetInfo tar=service.selectTargetData(info.getList().get(i).getFk_topic_id());
					if(tar!=null)
				    info.getList().get(i).setLanmu_name(tar.getTopic_name());}
			}
			return info;
		} catch (Exception e) {
			e.printStackTrace();
